...
#define FXP_MAX_LFN 255            // max size of a long file name
#define FTP_CMD_SIZE FXP_MAX_LFN+8 // max size of a command
#define FTP_CWD_SIZE FXP_MAX_LFN+8 // max size of a directory name
#define FTP_FIL_SIZE FXP_MAX_LFN   // max size of a file name
...
public:
  ...
  void    begin( const char * _user, const char * _pass, const char * welcomeMessage = "Welcome to LilyGo-T-Dongle-S3 FTP server" );
  void    begin( const char * welcomeMessage = "  -- Welcome to LilyGo-T-Dongle-S3 FTP server" );
  void setFileSystem(fs::FS& fs);
  void setStorageType(uint8_t storageType);
...
private:
  ...
  bool     exists( const char * path ) {
	  ...
	  FTP_DIR f = currentFS->open(path, "r");
	  return (f == true);
#else
	  return currentFS->exists( path );
#endif
  };
  bool     remove( const char * path ) { return currentFS->remove( path ); };
#if STORAGE_TYPE == STORAGE_SPIFFS
  bool     makeDir( const char * path ) { return false; };
  bool     removeDir( const char * path ) { return false; };
#else
  bool     makeDir( const char * path ) { return currentFS->mkdir( path ); };
  bool     removeDir( const char * path ) { return currentFS->rmdir( path ); };
#endif

#if (STORAGE_TYPE == STORAGE_SD || STORAGE_TYPE == STORAGE_SD_MMC) && !defined(ESP32)
  bool     rename( const char * path, const char * newpath );
#else
  bool     rename( const char * path, const char * newpath ) { return currentFS->rename( path, newpath ); };
#endif
