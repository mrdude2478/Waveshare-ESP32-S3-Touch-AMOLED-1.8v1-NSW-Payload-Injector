...
fs::FS* currentFS = &FFat; // Default to FFat
uint8_t selectedStorageType = 0;
	
void FtpServer::setFileSystem(fs::FS& fs) {
    currentFS = &fs;
}

void FtpServer::setStorageType(uint8_t storageType) {
    selectedStorageType = storageType;
    DEBUG_PRINT(F("Storage type set to: "));
    DEBUG_PRINTLN(storageType);
}

void FtpServer::updateFileSystem() {
    if (selectedStorageType == STORAGE_SD) {
        currentFS = &SD; // Default to S
        DEBUG_PRINTLN(F("File system switched to SD"));
    } else if (selectedStorageType == STORAGE_FFAT) {
        currentFS = &FFat; // Default to FFat
        DEBUG_PRINTLN(F("File system switched to FFAT"));
    } else {
        // Fallback to default storage type for the platform
        #if defined(ESP32)
            currentFS = &FFat; // Default to FFat
        #elif defined(ESP8266)
            currentFS = &LittleFS; // Default for ESP8266
        #else
            currentFS = &SPIFFS; // Default for other platforms
        #endif
        DEBUG_PRINTLN(F("File system set to default"));
    }
    // Reset current directory to root to ensure valid state
    strcpy(cwdName, "/");
}

bool shouldSkipFile(const String& filename) {
    // Skip config.json in any directory
    String baseName = filename;
    baseName.toLowerCase();
    if (baseName.endsWith("/config.json")) return true;
...
void FtpServer::clientConnected()
{
  DEBUG_PRINTLN(F(" Client connected!"));
	...
	client.print(F("220 -- ESP32 RCM Dongle Server "));
	client.print(F(", Version ")); client.print(FTP_SERVER_VERSION); client.println(F(") --"));
  iCL = 0;
  if (FtpServer::_callback) {
  ...
  else if( CommandIs( "SITE" ))
{
    ...
    else if( ParameterIs( "STORAGE SD" ))
    {
        FtpServer::setStorageType(STORAGE_SD);
        updateFileSystem();
        client.println(F("200 Storage type set to SD"));
    }
    else if( ParameterIs( "STORAGE FFAT" ))
    {
        setStorageType(STORAGE_FFAT);
        updateFileSystem();
        client.println(F("200 Storage type set to FFAT"));
    }
    else {
        client.print( F("500 Unknown SITE command ") ); client.println( parameter );
    }
...
bool FtpServer::openDir( FTP_DIR * pdir )
{
 ...
 if( strlen( cwdName ) == 0 ){
    dir = currentFS->openDir( "/" );
  } else {
    dir = currentFS->openDir( cwdName );
  }
  openD = dir.rewind();
 ...
 if( strlen( cwdName ) == 0 ){
	 ...
	 if( strlen( cwdName ) == 0 ){
	  	  ...
	  	  FTP_DIR d = currentFS->open( "/" );
		  dir=d;
	  } else {
		  DEBUG_PRINT("cwdName -> ");
		  DEBUG_PRINTLN(cwdName );

		  FTP_DIR d = currentFS->open( cwdName );
		  dir=d;
	  }
...
bool FtpServer::doList() {
...
#endif
  {
    ...
    if (shouldSkipFile(fn)) return doList();
    ...
    const char* fn = (fnC[0] == '/') ? &fnC[1] : fnC;
    if (shouldSkipFile(String(fn))) {
      fileDir.close();
      return doList();
    }
...
#else
#if STORAGE_TYPE == STORAGE_SEEED_SD
  FTP_FILE fileDir = currentFS->open(dir.name());
...
#endif
  {
    ...
    if (shouldSkipFile(aza)) return doList();
    long fz = long(dir.fileSize());
    time_t time = dir.fileTime();
  ...
  if (fileDir) {
    ...
    if (fn[0] == '/') fn.remove(0, fn.lastIndexOf("/") + 1);
    if (shouldSkipFile(fn)) {
      fileDir.close();
      return doList();
    }
    generateFileLine(&data, fileDir.isDirectory(), fn.c_str(), fileDir.size(), "Jan 01 00:00", this->user);
    nbMatch++;
    return true;
  }
...
bool FtpServer::doMlsd() {
...
#endif
  {
    ...
    if (shouldSkipFile(fn)) return doMlsd();
...
#else
#if STORAGE_TYPE == STORAGE_SEEED_SD
  File fileDir = currentFS->open(dir.name());
  fileDir = dir.openNextFile();
#else
  File fileDir = dir.openNextFile();
#endif
...
uint32_t FtpServer::fileSize( FTP_FILE & file ) {
  ...
  bool FtpServer::openFile( char path[ FTP_CWD_SIZE ], int readTypeInt ){
		...
		file = currentFS->open( path, readTypeInt );
  ...
  bool FtpServer::openFile( const char * path, const char * readType ) {
  		...
  		file = currentFS->open( path, readType );
...
bool FtpServer::isDir( char * path )
{
	...
	return false; // no directory support
	  ...
	  FTP_DIR dir;
	  bool res;
	  dir = currentFS->open( path );

//	  return true;
	  res = dir.isDirectory();
	  return res;
#elif STORAGE_TYPE == STORAGE_FATFS
  return currentFS->isDir( path );
...
//  return currentFS->timeStamp( path, year, month, day, hour, minute, second );
#elif STORAGE_TYPE == STORAGE_FATFS
  return currentFS->timeStamp( path, year, month, day, hour, minute, second );
#else
  FTP_FILE file;
...
bool FtpServer::getFileModTime( char * path, uint16_t * pdate, uint16_t * ptime )
{
  ...
  return currentFS->getFileModTime( path, pdate, ptime );
#else
//  FTP_FILE file;
  bool res;
...
bool FtpServer::getFileModTime( uint16_t * pdate, uint16_t * ptime )
{
  ...
  * ptime = d.lastWriteTime;
  ...
  bool     FtpServer::rename( const char * path, const char * newpath ){
		...
		FTP_FILE myFileIn = currentFS->open(path, FILE_READ);
		FTP_FILE myFileOut = currentFS->open(newpath, FILE_WRITE);
		...
		if(myFileOut) {
			while (myFileIn.available() > 0)
				...
				myFileOut.close();
				myFileOut = currentFS->open(newpath, FILE_READ);
